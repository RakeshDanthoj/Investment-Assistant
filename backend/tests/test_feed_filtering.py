...
@patch("app.services.feed._fetch_feed_bundle_conn", return_value=([], True))
...
def test_build_feed_splits_category_param(
    ...
    assert mock_bundle.call_args.kwargs["categories"] == ["macro", "rbi_policy"]
...
def test_build_feed_loads_session_profile(
    ...
    mock_bundle.assert_called_once()
    assert mock_bundle.call_args.kwargs["profile"] == prof
    assert mock_bundle.call_args.kwargs["horizon_override"] is None
